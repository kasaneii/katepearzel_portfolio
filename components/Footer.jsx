...
const Footer = () => {
  ...
  return (
    <footer className={`${styles.xPaddings} py-8 w-full bg-mineshaft`}>
      <div className='text-honey font-light flex justify-between'>
        ...
        <div className={`${styles.flexEnd} gap-10`}>
          {footerLinks.map((link) => (
            <Link target='_blank' key={link.id} href={link.url}>
              <p className='links pb-5 text-[14px]' onMouseEnter={handleMouse} onMouseLeave={handleMouseLeave}>
                {link.link}
